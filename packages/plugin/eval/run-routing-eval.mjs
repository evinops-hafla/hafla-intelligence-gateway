...
//   (default)          the real routing eval. Needs a model key — ANTHROPIC_API_KEY (canonical: the
...
for (const s of skillDirs) {
  ...
  if (!d) {
    console.error(`✗ could not read description for ${s}`);
    process.exit(1);
  }
...
const golden = JSON.parse(
  readFileSync(join(HERE, 'golden-routing.json'), 'utf8')
);
...
for (const [i, c] of cases.entries()) {
  if (!c.question || typeof c.question !== 'string')
    structErrors.push(`case ${i}: missing/invalid question`);
  if (!validTargets.has(c.expectedSkill))
    structErrors.push(
      `case ${i} ("${c.question}"): expectedSkill "${c.expectedSkill}" is not a real skill or "${OUT_OF_SCOPE}"`
    );
...
for (const s of skillDirs)
  if (!covered.has(s))
    structErrors.push(`skill "${s}" has no golden case — add at least one`);

if (structErrors.length) {
  console.log(
    `✗ golden-routing.json structural errors (${structErrors.length}):`
  );
  for (const e of structErrors) console.log(`   - ${e}`);
  process.exit(1);
}
console.log(
  `✓ structure OK — ${cases.length} cases, all 6 skills covered, ${cases.filter((c) => c.negative).length} seeded negatives, ${cases.filter((c) => c.expectedSkill === OUT_OF_SCOPE).length} out-of-scope.`
);
...
if (!PROVIDER) {
  ...
  console.log(
    '   ANTHROPIC_API_KEY=…  (canonical — the production router is Claude), or'
  );
  console.log(
    '   GEMINI_API_KEY=…  /  GOOGLE_API_KEY=…  (Google AI Studio — a cross-model proxy).'
  );
  process.exit(0);
}
if (PROVIDER !== 'anthropic') {
  ...
  console.log('   (ANTHROPIC_API_KEY) stays the canonical Tier-1 measure.');
}

const catalogue = skillDirs
  .map((s) => `- ${s}: ${descriptions.get(s)}`)
  .join('\n');
...
for (const c of cases) {
  let got;
  try {
    got = await route(c.question);
  } catch (e) {
    console.error(`✗ API error on "${c.question}": ${e.message}`);
    process.exit(1);
  }
  ...
  if (verbose)
    console.log(
      `   ${ok ? '✓' : '✗'} ${c.question}  →  ${got}${ok ? '' : ` (expected ${c.expectedSkill})`}`
    );
}
...
if (misses.length) {
  ...
  for (const m of misses)
    console.log(
      `   ✗ "${m.q}"${m.negative ? ' [seeded negative]' : ''}\n       expected ${m.expected}, got ${m.got}`
    );
}
if (acc < THRESHOLD) {
  console.log(
    `\n✗ FAIL — accuracy ${(acc * 100).toFixed(1)}% < ${THRESHOLD * 100}% threshold`
  );
  process.exit(1);
}
console.log(
