...
> **Coexists with the OAuth Web connector.** The gateway now also has a browser-OAuth "Web connector" (WorkOS AuthKit) for claude.ai / Claude Desktop / Claude Code — built + enabled + staging-verified live (see the plugin's `DESKTOP-SETUP.md`). It is **not** a replacement: this bridge stays the canonical path for Cursor / Gemini CLI / Antigravity / automation and for anyone using their `gcloud` identity. Nothing here changes as the connector rolls out (production GA pending).
