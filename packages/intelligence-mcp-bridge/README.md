...
**23 read-only tools**, all backed by Hafla's data lakes + identity layer (live at `mcp.hafla.com`). They fall into seven groups:
...
| `safe_sql_sandbox_core`  | Read-only SQL against the **live** Hafla Core RDS primary — current-second freshness; 30 s timeout    |
| `safe_cypher_sandbox`    | Read-only Cypher over the 838 K-node Neo4j identity/event graph (hosts, events, orders, products, partners, tickets…) |
...
| `search_internal_knowledge` | Semantic search over the WhatsApp / Slack conversation corpus (Vertex AI)                           |
...
| `seasonal_demand`   | Monthly booking or demand curve for UAE events, with index vs the yearly mean                             |
