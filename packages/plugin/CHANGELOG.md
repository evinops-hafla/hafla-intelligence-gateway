...
- Docs/skills — post-review hardening pass (2026-09-07). Reconciled the OAuth docs so Claude Code (CIMD, first verified 2026-09-07 on one machine) is no longer conflated with the claude.ai/Desktop Web connector (DCR, GA 2026-09-05); fixed a staging-vs-prod GA wording contradiction in `DESKTOP-SETUP.md`; made the `SKILLS-GUIDE.md` troubleshooting fallback OAuth-aware; root README banner now cites both surfaces/dates. Skills: the three drift-survey tool caveats — `price_truth.lastOrderedAt`, `related_products.rankBy`/`liftReliable`, and `get_org_events.additionalDomains` (merge/dedup) — were **live-verified against the gateway 2026-09-07**, and `additionalDomains` now ships as a real `get_org_events({ … })` call literal so `verify-skills.mjs` covers the param. Docs + skill-caveat edits only; no version bump.
